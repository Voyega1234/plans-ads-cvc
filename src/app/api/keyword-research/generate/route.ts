...
function findPlannerMatch(keyword: string, plannerMap: Map<string, PlannerKeyword>): PlannerKeyword | undefined {
...
ตอบ JSON เท่านั้น:
{
  ...
  if (usage) {
    ...
    void logAiCost({ route: '/api/keyword-research/generate', model: `vertex:${modelName}`, inputTokens: inp, outputTokens: out, estimatedUSD: (inp / 1e6) * 0.075 + (out / 1e6) * 0.30, provider: usageLabels.provider, feature: usageLabels.feature, subfeature: usageLabels.subfeature })
  }
  ...
  const keywords = parsed.keywords.map((k: unknown) => {
    ...
    const group = rawGroup === 'high_intent' || rawGroup === 'problem_intent'
      ? 'service'
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { businessName, productService, location, objective, competitors, customerId: inputCustomerId } = input
    ...
    const { keywords: aiKeywords, negativeKeywords } = await buildKeywordsWithAI(businessName, productService, location, objective, competitors ?? '')
    ...
    const plannerMap = await fetchFromKeywordPlanner(allSeeds, accessToken ?? '', customerId, location)
